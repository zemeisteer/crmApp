...
function CertificatesContent() {
  ...
  return (
    <div className="space-y-6">
      {/* Top action banner */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/60 border border-slate-800/80 p-6 rounded-2xl backdrop-blur-xl">
        <div>
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold inline-flex items-center gap-2 shadow-lg shadow-indigo-600/20 transition-all cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
        <div className="sm:col-span-8 relative">
          <input
            ...
            className="w-full bg-slate-900/80 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
      ...
      {/* Main content list */}
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-sm">Sertifikatlar yuklanmoqda...</div>
      ) : filtered.length === 0 ? (
        <div className="p-12 text-center bg-slate-900/40 border border-slate-800/60 rounded-2xl">
          ...
          <p className="text-slate-300 font-medium text-sm">Hech qanday sertifikat topilmadi</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {paged.map((cert) => (
            <div
              key={cert.id}
              className="bg-slate-900/80 border border-slate-800/80 hover:border-indigo-500/40 rounded-2xl p-5 backdrop-blur-xl transition-all shadow-lg flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-3 mb-3">
                  ...
                  {cert.grade && (
                    <span className="text-xs font-bold text-indigo-300 bg-indigo-500/10 border border-indigo-500/30 px-2.5 py-1 rounded-lg shrink-0">
                      {cert.grade}
                    </span>
                  )}
                </div>
              ...
              <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Link
                    ...
                    target="_blank"
                    className="px-3 py-1.5 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 text-xs font-medium inline-flex items-center gap-1.5 transition-all"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium inline-flex items-center gap-1.5 transition-all cursor-pointer"
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">
              O'quvchi <span className="text-red-400">*</span>
            </label>
            <Select
              value={studentId}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">
              Sertifikat Nomi <span className="text-red-400">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div className="pt-3 flex justify-end gap-2 border-t border-slate-800">
            ...
            >
              {saving ? "Saqlanmoqda..." : "Sertifikatni Berish"}
            </button>
          </div>
        </form>
      </Modal>
