...
import { teachersApi, groupsApi, paymentsApi, Teacher, Group, Student, Payment, ApiError } from "@/lib/api";
...
function TeacherDetailContent() {
  ...
  async function onDelete() {
    if (!confirm("O'qituvchini butunlay o'chirishni tasdiqlaysizmi?")) return;
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: "#8A8D96", marginTop: 2 }}>
                {teacher.subject || "Yo'nalish kiritilmagan"}
      ...
      <div style={{ flex: 1, minHeight: 0, padding: "26px 32px", display: "flex", flexDirection: "column", gap: 20, overflow: "auto", boxSizing: "border-box" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 16 }}>
          <div style={{ background: "#fff", border: "1px solid #EAE8E2", borderRadius: 14, padding: 18 }}>
            <div style={{ fontSize: 12, color: "#8A8D96" }}>Guruhlar</div>
            <div style={{ fontSize: 24, fontWeight: 800, fontFamily: "'Manrope', sans-serif", marginTop: 4 }}>{fullGroups.length}</div>
          </div>
          <div style={{ background: "#fff", border: "1px solid #EAE8E2", borderRadius: 14, padding: 18 }}>
            <div style={{ fontSize: 12, color: "#8A8D96" }}>Jami o&apos;quvchilar</div>
            <div style={{ fontSize: 24, fontWeight: 800, fontFamily: "'Manrope', sans-serif", marginTop: 4 }}>{totalStudents}</div>
          </div>
          <div style={{ background: "#fff", border: "1px solid #EAE8E2", borderRadius: 14, padding: 18 }}>
            <div style={{ fontSize: 12, color: "#8A8D96" }}>Bu oy guruh tushumi</div>
            <div style={{ fontSize: 24, fontWeight: 800, fontFamily: "'Manrope', sans-serif", marginTop: 4 }}>{formatMoney(totalRevenue)} so&apos;m</div>
          </div>
          <div style={{ background: "#ECEBFB", border: "1px solid #D7D3F8", borderRadius: 14, padding: 18 }}>
            <div style={{ fontSize: 12, color: ACCENT }}>Bu oy hisoblangan maosh</div>
            <div style={{ fontSize: 24, fontWeight: 800, fontFamily: "'Manrope', sans-serif", marginTop: 4, color: ACCENT }}>
              {teacher.salaryValue ? `${formatMoney(calculatedSalary)} so'm` : "—"}
            </div>
          </div>
        </div>

        <div style={{ background: "#fff", border: "1px solid #EAE8E2", borderRadius: 16, padding: 20 }}>
          <div style={{ fontFamily: "'Manrope', sans-serif", fontWeight: 700, fontSize: 15, marginBottom: 14 }}>O&apos;qituvchi haqida</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: "18px 16px" }}>
            <InfoField label="Email" value={teacher.email || "—"} />
            <InfoField label="Telefon" value={teacher.phone || "—"} />
            <InfoField
              label="Maosh turi"
              value={teacher.salaryType === "PERCENT" ? "Foizli (guruh tushumidan)" : "Belgilangan (oylik)"}
            />
            <InfoField
              label={teacher.salaryType === "PERCENT" ? "Foiz" : "Oylik maosh"}
              value={teacher.salaryValue ? (teacher.salaryType === "PERCENT" ? `${teacher.salaryValue}%` : `${formatMoney(teacher.salaryValue)} so'm`) : "—"}
            />
          </div>
        </div>

        <div style={{ background: "#fff", border: "1px solid #EAE8E2", borderRadius: 16, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px 4px", fontFamily: "'Manrope', sans-serif", fontWeight: 700, fontSize: 15 }}>Guruhlari</div>
          {fullGroups.length === 0 ? (
            <div style={{ color: "#8A8D96", fontSize: 14, padding: "24px 20px" }}>Hali biror guruhga biriktirilmagan.</div>
          ) : (
            <table>
              <thead>
                <tr>
                  <th style={{ paddingTop: 14 }}>Guruh</th>
                  <th style={{ paddingTop: 14 }}>Jadval</th>
                  <th style={{ paddingTop: 14 }}>O&apos;quvchilar</th>
                  <th style={{ paddingTop: 14 }}>Bu oy tushum</th>
              ...
              <tbody>
                {fullGroups.map((g) => (
                  <tr key={g.id}>
                    ...
                    <td>{formatMoney(groupRevenue(g))} so&apos;m</td>
